...
Enables natural language queries for flight schedule optimization.
"""
...
try:
    ...
    print("Warning: scikit-learn not available for advanced NLP features.")
...
class NLPQueryProcessor:
    """
    Processes natural language queries for flight schedule optimization.
    """
    ...
    def _initialize_nlp(self):
        """Initialize spaCy NLP model with fallback."""
        if SPACY_AVAILABLE:
            try:
                self.nlp = spacy.load("en_core_web_sm")
            except OSError:
                print("Warning: spaCy English model not found. Using basic processing...")
                self.nlp = None
        else:
            print("Warning: spaCy not available. Using basic tokenizer...")
            self.nlp = None
    ...
    def generate_sql_query(self, intent: QueryIntent, table_name: str = 'flights') -> str:
        """
        ...
        if intent.entity == 'delays':
            select_clause = "SELECT Flight_ID, Airline, Delay_Minutes, Scheduled_Time"
        elif intent.entity == 'runways':
            select_clause = "SELECT Runway, COUNT(*) as Flight_Count, AVG(Delay_Minutes) as Avg_Delay"
        elif intent.entity == 'airlines':
            select_clause = "SELECT Airline, COUNT(*) as Flight_Count, AVG(Delay_Minutes) as Avg_Delay"
        else:
        ...
        if 'time' in intent.filters:
            time_filter = intent.filters['time']
            if time_filter == 'tomorrow':
                where_conditions.append("DATE(Scheduled_Time) = DATE('now', '+1 day')")
            elif time_filter == 'today':
                where_conditions.append("DATE(Scheduled_Time) = DATE('now')")
            elif time_filter == 'morning':
                where_conditions.append("HOUR(Scheduled_Time) BETWEEN 6 AND 11")
            elif time_filter == 'evening':
                where_conditions.append("HOUR(Scheduled_Time) BETWEEN 18 AND 23")
            elif time_filter == 'peak_hours':
                where_conditions.append("(HOUR(Scheduled_Time) BETWEEN 7 AND 9 OR HOUR(Scheduled_Time) BETWEEN 18 AND 20)")
        
        if 'airlines' in intent.filters:
            airline_list = "', '".join(intent.filters['airlines'])
            where_conditions.append(f"LOWER(Airline) IN ('{airline_list}')")
        
        if 'airports' in intent.filters:
            airport_list = "', '".join(intent.filters['airports'])
            where_conditions.append(f"(Origin IN ('{airport_list}') OR Destination IN ('{airport_list}'))")
        
        if 'delay_threshold' in intent.filters:
            threshold = intent.filters['delay_threshold']
            where_conditions.append(f"Delay_Minutes > {threshold}")
        
        if where_conditions:
            sql += " WHERE " + " AND ".join(where_conditions)
        
        # GROUP BY clause for aggregated queries
        if intent.entity in ['runways', 'airlines']:
            if intent.entity == 'runways':
                sql += " GROUP BY Runway"
            elif intent.entity == 'airlines':
                sql += " GROUP BY Airline"
        
        # ORDER BY clause based on modifiers
        if 'quantity' in intent.filters:
            if intent.filters['quantity'] == 'most':
                if intent.entity == 'delays':
                    sql += " ORDER BY Delay_Minutes DESC"
                elif intent.entity in ['runways', 'airlines']:
                    sql += " ORDER BY Avg_Delay DESC"
            elif intent.filters['quantity'] == 'least':
                if intent.entity == 'delays':
                    sql += " ORDER BY Delay_Minutes ASC"
                elif intent.entity in ['runways', 'airlines']:
                    sql += " ORDER BY Avg_Delay ASC"
        
        # LIMIT clause
        if 'quantity' in intent.filters and intent.filters['quantity'] in ['most', 'least']:
            sql += " LIMIT 10"
